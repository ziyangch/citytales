...
Page({
  ...
  markerTap: function(event) {
    ...
    let current_story = this.data.stories.find(
      story => story.id == event.markerId
    );
    this.setData({
      current_story
    });
    ...
    let index = this.data.stories.findIndex(
      story => story.id == event.markerId
    );
  ...
  mapTap: function(event) {
    let markers = this.data.markers;
    let index = this.data.stories.findIndex(
      story => story.id == this.data.current_story.id
    );
  ...
  setMarkers: function(stories) {
    let markers = stories.map(story => {
      return {
        ...
        name: story.title,
        iconPath:
          "https://cloud-minapp-32027.cloud.ifanrusercontent.com/1ifOM5c01ybmL4zj.png",
        width: 40,
        height: 40
      };
    });
  ...
  savePolylineToBackend: function() {
    ...
    let polylineLatitudeArr = polyline[0].points.map(point => {
      return point.latitude;
    });
    console.log("polylineLatitudeArr ------>", polylineLatitudeArr);
    let polylineLongitudeArr = polyline[0].points.map(point => {
      return point.longitude;
    });
    console.log("polylineLongitudeArr ------>", polylineLongitudeArr);
    console.log(
      "typeof polylineLongitudeArrayElement ------->",
      typeof polylineLongitudeArr[5]
    );
    ...
    qqmapsdk.calculateDistance({
      ...
      complete: function(res) {
        ...
        if (!(that.data.topTags === undefined)) {
          ...
          let filteredForRecommendation = filteredByProximity.filter(function(
            item
          ) {
            return item.people_liked > 5;
          });
          console.log(
            "filteredForRecommendation pre sort---->",
            filteredForRecommendation
          );
          filteredForRecommendation.sort(
            (a, b) => b.people_liked - a.people_liked
          );
          console.log(
            "filteredForRecommendation post sort---->",
            filteredForRecommendation
          );
          that.setData({
  ...
  getUserPreferences: function(userId) {
    ...
    UserStory.setQuery(query)
      ...
      .then(res => {
        ...
        let specStories = filteredSpecUserStories.map(
          filteredSpecUserStory => filteredSpecUserStory.story
        );
        ...
        let userPrefs = [
          // (2) counting occurrence of tags within storiesSaved array
          {
            name: "建筑",
          ...
          {
            name: "游览",
            occurrence: this.getOccurrence(userTagsArray, "游览")
          },
          {
            name: "历史",
            occurrence: this.getOccurrence(userTagsArray, "历史")
          },
          {
            name: "感想",
            occurrence: this.getOccurrence(userTagsArray, "感想")
          },
          {
            name: "故事",
            occurrence: this.getOccurrence(userTagsArray, "故事")
          },
          {
            name: "摄影",
            ...
            posterConfig: {
              ...
              images: [
                ...
                },
                {
                  ...
                  y: 40,
                  url:
                    "https://cloud-minapp-32027.cloud.ifanrusercontent.com/1ifh3hlco9fcu01w.PNG"
                  //邮票二维码
                }
              ]
            }
  ...
  onShareAppMessage: function() {
    return {
      ...
      imageUrl:
        "https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1576076902204&di=3976f55fd2511190201063cb611dbfc1&imgtype=0&src=http%3A%2F%2Fpic5.997788.com%2Fpic_search%2F00%2F16%2F10%2F15%2Fse16101588a.jpg"
    };
  },
  ...
  onChange(e) {
    ...
    if (e.currentTarget.id === "segmented-control") {
      ...
      let filteredByTags = filteredByDistance.filter(function(item) {
        // check if there is any overlap in the two arrays (chosen filter and story tags)
      ...
      if (distanceSorter === (1 || -1)) {
        ...
        if (commentsSorter === -1) {
          filteredByTags.sort(
            (a, b) => a.people_commented - b.people_commented
          );
        } else {
          filteredByTags.sort((a, b) => a.distance - b.distance);
        }
